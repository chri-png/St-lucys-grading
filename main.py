...
def result_to_item(r: models.Result) -> schemas.ResultItem:
    return schemas.ResultItem(term=r.term, subject=r.subject, score=r.score)
...
def teacher_add_result(
    ...
    result = models.Result(
        student_id=student.id, term=payload.term, subject=payload.subject, score=payload.score
    )
    db.add(result)
    db.commit()
